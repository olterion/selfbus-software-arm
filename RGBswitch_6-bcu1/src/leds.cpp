...
void initLEDs() {
	...
	if (function_leds_active) {
		...
		for (uint8_t i = 0; i < LEDS / 3; i++) {
			...
			rgb_buffer_vorgabe[rgb_leds[i].blau] = farbauswahl[led_color_param].blau;
			rgb_buffer_vorgabe[rgb_leds[i].rot] = farbauswahl[led_color_param].rot;
			rgb_buffer_vorgabe[rgb_leds[i].gruen] = farbauswahl[led_color_param].gruen;
		}
	}
	...
	if (nightlight_configured) {
		...
		rgb_buffer_nachtlicht[rgb_leds[0].blau] = farbauswahl[LEDparams[2] >> 4].blau;
		rgb_buffer_nachtlicht[rgb_leds[0].rot] = farbauswahl[LEDparams[2] >> 4].rot;
		rgb_buffer_nachtlicht[rgb_leds[0].gruen] = farbauswahl[LEDparams[2] >> 4].gruen;
	}
